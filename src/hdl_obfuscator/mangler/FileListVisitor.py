...
class FileListVisitor:
    ...
    def _read_file_list(self, file_list_path) -> set[str]:
        source_files = set()
        with open(file_list_path, "r", encoding="utf-8") as obfuscation_list:
            for line in obfuscation_list.readlines():
                ...
                if self._is_verilog_file_path(line):
                    ...
                    line = os.path.expandvars(os.path.normpath(line))
                    source_files.add((line, line))

                elif self._is_incdir_path(line):
                    incdir_line = self._get_file_list_from_incdir_path(line)
                    incdir_files_path = os.path.expandvars(os.path.normpath(incdir_line))
                    ...
                    for root, _, files in os.walk(incdir_files_path):
                        for file in files:
                            ...
                            source_files.add((file_path,file_path))

                elif self._is_file_list_file_path(line):
                    inner_file_list_path = os.path.expandvars(os.path.normpath(self._get_file_list_from_file_path(line)))
                    ...
                    source_files.add((inner_source_files,inner_source_files))
    ...
    def _get_file_list_from_incdir_path(self, path: str) -> str:
        if path.startswith("+incdir+"):
            return path[len("+incdir+"):]  # Remove only the `+incdir+` part
